...
class AssemblyLLM:
    ...
    def _get_prompt_files(self):
        ...
        prompt_files["system_prompt"] = self.llm_config.get("prompt_files").get("system_prompt")
        prompt_files["examples_prompt"] = self.llm_config.get("prompt_files").get("examples_prompt")
        prompt_files["example_image_paths"] = self.llm_config.get("prompt_files").get("example_image_paths")
...
class AssemblyLLMOpenAI:
    ...
    def _get_prompt_files(self):
        ...
        prompt_files["example_rgb_paths"] = self.config.get("relation").get("dist_prompt_files").get("example_rgb_paths")
        prompt_files["example_depth_paths"] = self.config.get("relation").get("dist_prompt_files").get("example_depth_paths")
        prompt_files["example_seg_paths"] = self.config.get("relation").get("dist_prompt_files").get("example_seg_paths")
        ...
        if seg_image_paths:
            ...
            for ex_id, example in enumerate(examples):
                messages[-1]["content"] += [
                    ...
                    {"type": "text", "text": f"Expected output:\n```{example['output']}```\n\n"}
                ]
            
            messages.append({
                "role": "user",
                "content": [
                    {"type": "text", "text": "Now, based on the task description and the examples above, respond to this query:"},
            ...
            messages.append({
                ...
                "content": [
                    # {"type": "text", "text": (
                    #     "You are given RGB images of a table with building blocks. Your task is to generate a directed graph that represents contact information as relationships between these blocks. "
                    #     "This directed graph will be used to reconstruct a queried structure. Follow these steps:\n\n"
                    #     "1. Detect individual building blocks on the table in the input image.\n"
                    #     "2. Assign a unique `block_id` and `color` to each block.\n"
                    #     "3. Generate a directed graph. Nodes are the blocks. Edges represent relative 3D distances from one block to another.\n"
            ...
            messages.append({
                "role": "user",
                "content": [
                    # {"type": "text", "text": f"I want you to generate a directed graph that represents contact information as relationships between blocks on the table shown in the input image. This directed graph will encode the structural information needed to build the input query structure. Do it step by step. First, detect each building block on the table in the input image. Second, Assign a unique `block_id` and `color` to individual building blocks. Third, generate a directed graph with nodes of the building blocks and edges of relative distances from one block to another. Note that x is back to front, y is left to right, and z is bottom to up. For reference, I will give you some examples."}
                    {"type": "text", "text": f"I want you to generate a directed graph that represents contact information as relationships between blocks on the table. This directed graph will encode the structural information needed to build the input query structure. Follow these steps: 1. Detect each building block on the table in the input image. 2. Assign a unique `block_id` and `color` to individual building blocks. 3. Generate a directed graph with nodes of the building blocks and edges of relative distances from one block to another. Note that x is back to front, y is left to right, and z is bottom to up. For reference, I will give you some examples.\n\n"}
                ]
            })
