...
class AssemblyLLM:
    ...
    def _get_prompt_files(self):
        ...
        prompt_files["example_mask_paths"] = self.config.get("relation").get("prim_prompt_files").get("example_mask_paths")
...
class AssemblyLLMOpenAI:
    ...
    def _get_prompt_files(self):
        ...
        # prompt_files["example_depth_paths"] = self.config.get("relation").get("prim_prompt_files").get("example_depth_paths")
        # prompt_files["example_mask_paths"] = self.config.get("relation").get("prim_prompt_files").get("example_mask_paths")
        prompt_files["example_rgbd_paths"] = self.config.get("relation").get("prim_prompt_files").get("example_rgbd_paths")
        prompt_files["example_rgbseg_paths"] = self.config.get("relation").get("prim_prompt_files").get("example_rgbseg_paths")
        ...
        # prompt_files["example_mask_paths_dist"] = self.config.get("relation").get("dist_prompt_files").get("example_mask_paths")
        prompt_files["example_rgbd_paths_dist"] = self.config.get("relation").get("dist_prompt_files").get("example_rgbd_paths")
        prompt_files["example_rgbseg_paths_dist"] = self.config.get("relation").get("dist_prompt_files").get("example_rgbseg_paths")
        ...
        prompt_files["target_rgb_path"] = os.path.join(target_dir, self.target_config.get("target_rgb_file"))  # Color image to analyze
        prompt_files["target_depth_path"] = os.path.join(target_dir, self.target_config.get("target_depth_file"))  # Depth image to analyze
        prompt_files["target_mask_path"] = os.path.join(target_dir, self.target_config.get("target_mask_file"))  # Segmentation mask to analyze
        prompt_files["target_rgbd_path"] = os.path.join(target_dir, self.target_config.get("target_rgbd_file"))  # 4-Channel RGB-D to analyze
        prompt_files["target_rgbseg_path"] = os.path.join(target_dir, self.target_config.get("target_rgbseg_file"))  # blocks segmentation image to analyze
        
        return prompt_files

    # def _process_examples(self, examples_prompt, example_rgbs, example_depths, example_masks):
    def _process_examples(self, examples_prompt, example_rgbs, example_rgbds, example_rgbsegs):
        """
        Process raw examples into structured format
        """
        example_entries = examples_prompt.split('### Example')[1:]  # Skip first empty split
        ...
        for i, entry in enumerate(example_entries):
            ...
            # input_depth_path = example_rgbd_paths[i] if example_depth_paths else ""
            # input_mask_path = example_rgbd_paths[i] if example_mask_paths else ""
    ...
    # def _process_examples_for_distances(self, examples_prompt, example_rgbs, example_depths, example_masks):
    def _process_examples_for_distances(self, examples_prompt, example_rgbs, example_rgbds, example_rgbsegs):
        """
        Process raw examples into structured format
        """
    ...
    def _build_messages_openai(self):
        ...
        # mask_image_paths = read_file(self.prompt_files["example_mask_paths"]) if self.prompt_files["example_mask_paths"] else ""
        rgbd_image_paths = read_file(self.prompt_files["example_rgbd_paths"]) if self.prompt_files["example_rgbd_paths"] else ""
        rgbseg_image_paths = read_file(self.prompt_files["example_rgbseg_paths"]) if self.prompt_files["example_rgbseg_paths"] else ""

        # examples = self._process_examples(examples_prompt, rgb_image_paths, depth_image_paths, mask_image_paths)
        examples = self._process_examples(examples_prompt, rgb_image_paths, rgbd_image_paths, rgbseg_image_paths)
        ...
        # if len(rgb_image_paths) != 0 and len(depth_image_paths) == 0 and len(mask_image_paths) == 0:    # RGB without depth or mask
        if len(rgb_image_paths) != 0 and len(rgbd_image_paths) == 0 and len(rgbseg_image_paths) == 0:    # RGB without depth or mask
        ...
        # elif len(rgb_image_paths) != 0 and len(depth_image_paths) != 0 and len(mask_image_paths) == 0:    # RGB-D
        elif len(rgb_image_paths) == 0 and len(rgbd_image_paths) != 0 and len(rgbseg_image_paths) == 0:    # 4-channel RGB-D
            # Add few-shot examples
            for example in examples:
                # User message (image + query)
                messages.append({
        ...
        elif len(rgb_image_paths) == 0 and len(rgbd_image_paths) == 0 and len(rgbseg_image_paths) != 0:    # RGB block segmentation
            # Add few-shot examples
            for example in examples:
                # User message (image + query)
                messages.append({
    ...
    def _build_messages_openai_for_distances(self):
        ...
        # mask_image_paths = read_file(self.prompt_files["example_mask_paths_dist"]) if self.prompt_files["example_mask_paths_dist"] else ""
        rgbd_image_paths = read_file(self.prompt_files["example_rgbd_paths_dist"]) if self.prompt_files["example_rgbd_paths_dist"] else ""
        rgbseg_image_paths = read_file(self.prompt_files["example_rgbseg_paths_dist"]) if self.prompt_files["example_rgbseg_paths_dist"] else ""
        
        # examples = self._process_examples_for_distances(examples_prompt, rgb_image_paths, depth_image_paths, mask_image_paths)
        examples = self._process_examples_for_distances(examples_prompt, rgb_image_paths, rgbd_image_paths, rgbseg_image_paths)
        ...
        '''
        if len(rgb_image_paths) != 0 and len(depth_image_paths) != 0 and len(mask_image_paths) != 0:    # RGB-D-S
            messages.append({
                "role": "user",
                "content": [
        ...
        elif len(rgb_image_paths) != 0 and len(depth_image_paths) == 0 and len(mask_image_paths) != 0:    # RGB-S
        '''
        if len(rgb_image_paths) == 0 and len(rgbd_image_paths) == 0 and len(rgbseg_image_paths) != 0:    # RGB Segmentation of blocks
            messages.append({
                "role": "user",
                "content": [
        ...
        # elif len(rgb_image_paths) != 0 and len(depth_image_paths) == 0 and len(seg_image_paths) == 0:    # RGB without depth or mask
        elif len(rgb_image_paths) != 0 and len(rgbd_image_paths) == 0 and len(rgbseg_image_paths) == 0:    # simple RGB
            messages.append({
                "role": "user",
                "content": [
            ...
            assert len(rgb_image_paths) == 0 and len(rgbd_image_paths) == 0 and len(rgbseg_image_paths) == 0
