...
envs = [f"AssemblySimpleDemo{i}" for i in range(1, 5)]

for env_name in envs:
    env_id = int(env_name.split("AssemblySimpleDemo")[-1])
    ...
    # env.sim.model.cam_pos[cam_id] = np.array([-0.17, -0.19, 1.17])

    axisangle = np.array([np.pi / 2, 0, 0])
    # tilt = np.deg2rad(10)  # 10-degree adjustment
    # axisangle = np.array([np.pi / 2, tilt, 0])
    ...
    save_dir = os.path.join(save_root, f"icl_fmb_ex{env_id}")
    ...
    for cam_name in camera_names:
        ...
        if rgb_cam_name in obs.keys():
            ...
            save_path = os.path.join(save_dir, f"{cam_name}_frame.png")
