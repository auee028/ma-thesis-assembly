...
for i in range(configs["num_iter"]):
    ...
    if mode is False:    # When applying relation primitives        
        ...
        if example_rgbd_paths:  # Giving depth images in examples means target information also includes a depth image
        ...
        if example_rgbseg_paths:  # Giving segmentation masks in examples means target information also includes a mask image
            seg_cam_name = cam_name + "_segmentation_element"
            seg_img = obs[seg_cam_name]
            # save_path = configs["task"]["target_seg_path"]
            # save_cam_mask(seg_img, block_geom_ids, save_path)
            # save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_seg_{i}.png")
            # save_cam_mask(seg_img, block_geom_ids, save_path)
        ...
        # Save 4-channel RGB-D image
        if dist_example_rgbd_paths:  # Giving depth images in examples means target information also includes a depth image
            depth_cam_name = cam_name + "_depth"
            depth_img = obs[depth_cam_name]
            depth_img = get_real_depth_map(env.sim, depth_img)  # Convert MuJoCo's depth to real-world metric values
            # save_path = configs["task"]["target_depth_path"]
        ...
        # Save blocks' RGB segmentation image
        if dist_example_rgbseg_paths:  # Giving segmentation masks in examples means target information also includes a mask image
            seg_cam_name = cam_name + "_segmentation_element"
            seg_img = obs[seg_cam_name]
            # save_path = configs["task"]["target_seg_path"]
            # save_cam_mask(seg_img, block_geom_ids, save_path)
    ...
    if content != "None":
        ...
        if "structure_relations" in content.keys():
            ...
            spatial_dag, assembly_order = g(instructions)
            ...
            new_block_positions = planner._compute_block_positions(spatial_dag, assembly_order)
            print("New block positions: ", new_block_positions)
