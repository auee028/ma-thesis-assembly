...
for i in range(1):  # Capture 1 frames
    ...
    for cam_name in camera_names:
        ...
        if (rgb_cam_name in obs.keys()) and (depth_cam_name in obs.keys()):
            ...
            # Correct Pixels Shape (a batch of pixel locations and the corresponding depth image)
            u, v = np.meshgrid(np.arange(cam_w), np.arange(cam_h))
            pixels = np.stack((u, v), axis=-1).reshape(-1, 2)[np.newaxis, ...]  # Shape: (B, N, 2), here B=1, N=HxW
            
            # Correct Depth Map Shape
            depth_img = depth_img[np.newaxis, np.newaxis, ...]  # Shape: (B, 1, H, W), here B=1
            
            # Perform the Transformation
            points = transform_from_pixels_to_world(
                pixels=pixels,
                depth_map=depth_img,
                camera_to_world_transform=camera_transform
            ).reshape(-1, 3)
