...
if mode is False:    # When applying spatial relation primitives
    ...
    example_seg_paths = configs["relation"]["prim_prompt_files"]["example_seg_paths"]
    if example_seg_paths:
        os.system(f"cp {example_seg_paths} {os.path.join(save_dir, os.path.basename(example_seg_paths))}")
...
for i in range(configs["num_iter"]):
    ...
    # if dist_example_rgb_paths:  # Giving RGB images in examples means target information also includes an RGB image
    rgb_cam_name = cam_name + "_image"
    rgb_img = obs[rgb_cam_name]  # Get image from the observation
    save_path = configs["task"]["target_rgb_path"]
    save_cam_image(rgb_img, save_path)
    ...
    if mode is False:    # When applying relation primitives
        # Save depth image
        if example_depth_paths:  # Giving depth images in examples means target information also includes a depth image
        ...
        if example_seg_paths:  # Giving segmentation masks in examples means target information also includes a mask image
        ...
        # Save depth image
        if dist_example_depth_paths:  # Giving depth images in examples means target information also includes a depth image
            depth_cam_name = cam_name + "_depth"
            depth_img = obs[depth_cam_name]
            depth_img = get_real_depth_map(env.sim, depth_img)  # Convert MuJoCo's depth to real-world metric values
            save_path = configs["task"]["target_depth_path"]
        ...
        # Save segmentation mask image
        if dist_example_seg_paths:  # Giving segmentation masks in examples means target information also includes a mask image
            seg_cam_name = cam_name + "_segmentation_element"
            seg_img = obs[seg_cam_name]
            save_path = configs["task"]["target_seg_path"]
            save_cam_mask(seg_img, block_geom_ids, save_path)
