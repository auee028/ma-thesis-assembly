...
from robosuite.custom_utils.assembly_llm_fused_images import AssemblyLLM, AssemblyLLMOpenAI
...
config_path = "../configs/thesis_exp_configs_fused_images.json"
...
if env_config["env_name"].lowercase() != "sandwich":
env = suite.make(
...
if mode is False:    # When applying spatial relation primitives
    ...
    example_rgb_paths = configs["relation"]["prim_prompt_files"]["example_rgb_paths"]
    if example_rgb_paths:
        os.system(f"cp {example_rgb_paths} {os.path.join(save_dir, os.path.basename(example_rgb_paths))}")

    example_depth_paths = configs["relation"]["prim_prompt_files"]["example_depth_paths"]
    if example_depth_paths:
        os.system(f"cp {example_depth_paths} {os.path.join(save_dir, os.path.basename(example_depth_paths))}")

    example_mask_paths = configs["relation"]["prim_prompt_files"]["example_mask_paths"]
    if example_mask_paths:
        os.system(f"cp {example_mask_paths} {os.path.join(save_dir, os.path.basename(example_mask_paths))}")
        
    example_rgbd_paths = configs["relation"]["prim_prompt_files"]["example_rgbd_paths"]
    if example_rgbd_paths:
        os.system(f"cp {example_rgbd_paths} {os.path.join(save_dir, os.path.basename(example_rgbd_paths))}")
        
    example_rgbseg_paths = configs["relation"]["prim_prompt_files"]["example_rgbseg_paths"]
    if example_rgbseg_paths:
        os.system(f"cp {example_rgbseg_paths} {os.path.join(save_dir, os.path.basename(example_rgbseg_paths))}")
    ...
    dist_example_mask_paths = configs["relation"]["dist_prompt_files"]["example_mask_paths"]
    if dist_example_mask_paths:
        os.system(f"cp {dist_example_mask_paths} {os.path.join(save_dir, os.path.basename(dist_example_mask_paths))}")
        
    dist_example_rgbd_paths = configs["relation"]["dist_prompt_files"]["example_rgbd_paths"]
    if dist_example_rgbd_paths:
        os.system(f"cp {dist_example_rgbd_paths} {os.path.join(save_dir, os.path.basename(dist_example_rgbd_paths))}")
        
    dist_example_rgbseg_paths = configs["relation"]["dist_prompt_files"]["example_rgbseg_paths"]
    if dist_example_rgbseg_paths:
        os.system(f"cp {dist_example_rgbseg_paths} {os.path.join(save_dir, os.path.basename(dist_example_rgbseg_paths))}")
...
for i in range(configs["num_iter"]):        
    # Save images of the environment in each episode
    obs = env._get_observations()
    cam_name = configs["task"]["target_cam"]
    
    # Save RGB image (as default)
    # if example_rgb_paths:  # Giving RGB images in examples means target information includes only an RGB image and a query structure
    ...
    # Save target images
    if mode is False:    # When applying relation primitives
        # Save depth image and 4-channel RGB-D image
        if example_depth_paths or example_rgbd_paths:
            # Giving depth images in examples means target information also includes a depth image
            ...
            # Save depth image
            save_path = configs["task"]["target_depth_path"]
            save_cam_depth(depth_img, save_path)
            save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_depth_{i}.png")
            save_cam_depth(depth_img, save_path)
        
            if example_rgbd_paths:  # Giving RGBD images in examples means target information also includes an RGBD image
                # Save RGB-D image
                save_path = configs["task"]["target_rgbd_path"]
                save_fused_rgbd(rgb_img, depth_img, save_path)
                save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_rgbd_{i}.png")
                save_fused_rgbd(rgb_img, depth_img, save_path)
        
        # Save block segmentation mask and RGB segmentation image
        if example_mask_paths or example_rgbseg_paths:
            # Giving segmentation masks in examples means target information also includes a mask image
            mask_cam_name = cam_name + "_segmentation_element"
            mask_img = obs[mask_cam_name]
            
            # Save block segmentation mask
            save_path = configs["task"]["target_mask_path"]
            save_cam_mask(mask_img, block_geom_ids, save_path)
            save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_mask_{i}.png")
            save_cam_mask(mask_img, block_geom_ids, save_path)
            
            if example_rgbseg_paths:  # Giving RGBD images in examples means target information also includes an RGBD image
                # Save RGB block segmentation
                save_path = configs["task"]["target_rgbseg_path"]
        ...
        # Save depth image and 4-channel RGB-D image
        if dist_example_depth_paths or dist_example_rgbd_paths:
            # Giving depth images in examples means target information also includes a depth image
            depth_cam_name = cam_name + "_depth"
            depth_img = obs[depth_cam_name]
            depth_img = get_real_depth_map(env.sim, depth_img)  # Convert MuJoCo's depth to real-world metric values
            ...
            if dist_example_rgbd_paths:  # Giving RGBD images in examples means target information also includes an RGBD image
                # Save RGB-D image
                save_path = configs["task"]["target_rgbd_path"]
                save_fused_rgbd(rgb_img, depth_img, save_path)
                save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_rgbd_{i}.png")
        ...
        # Save block segmentation mask and RGB segmentation image
        if dist_example_mask_paths or dist_example_rgbseg_paths:
            # Giving segmentation masks in examples means target information also includes a mask image
            mask_cam_name = cam_name + "_segmentation_element"
            mask_img = obs[mask_cam_name]
            ...
            if dist_example_rgbseg_paths:  # Giving RGBD images in examples means target information also includes an RGBD image
                # Save RGB block segmentation
                save_path = configs["task"]["target_rgbseg_path"]
                save_fused_rgbseg(rgb_img, seg_img, block_geom_ids, save_path)
                save_path = os.path.join(save_dir, "assembly_input_img", f"assembly_rgbseg_{i}.png")
    ...
    content = llm(log_res_path=os.path.join(save_dir, "log_llm_res.txt"), num_iter=i)    # returns 'content' of response from LLM
    if content != "None":
