...
class BigPyramid(ManipulationEnv):
    """
    This class corresponds to the customized BigPyramid task for a single robot arm.
    ...
    def staged_rewards(self, action=None):
        ...
        higher_layer_offset = 0.04 * 2 + (0.04 / 2)
        lower_three = [abs(offset - lower_layer_offset) for offset in sorted_offsets[:3]]
        middle_two = [abs(offset - middle_layer_offset) for offset in sorted_offsets[3:5]]
        higher_one = [abs(offset - higher_layer_offset) for offset in sorted_offsets[5:]]

        scaled_r_lower = [-(offset / 0.04) if offset != 0.0 else 0 for offset in lower_three]
        scaled_r_middle = [-(offset / 0.04) if offset != 0.0 else 0 for offset in middle_two]
        scaled_r_higher = [-(offset / 0.04) if offset != 0.0 else 0 for offset in higher_one]
    ...
    def _load_model(self):
        ...
        self.cubeA = BoxObject(
            ...
            rgba=COLORNAME2RGBA["red"],
        )
        self.cubeB = BoxObject(
            name=f"cubeB",
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=COLORNAME2RGBA["orange"],
        )
        self.cubeC = BoxObject(
            name=f"cubeC",
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=COLORNAME2RGBA["yellow"],
        )
        self.cubeD = BoxObject(
            name=f"cubeD",
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=COLORNAME2RGBA["green"],
        )
        self.cubeE = BoxObject(
            name=f"cubeE",
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=COLORNAME2RGBA["blue"],
        )
        self.cubeF = BoxObject(
            name=f"cubeF",
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=COLORNAME2RGBA["purple"],
        )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [cubeA_pos, cubeA_quat, cubeB_pos, cubeB_quat, cubeC_pos, cubeC_quat, 
                       cubeD_pos, cubeD_quat, cubeE_pos, cubeE_quat, cubeF_pos, cubeF_quat]
