...
class TwinTowers(ManipulationEnv):
    """
    This class corresponds to the customized TwinTowers task for a single robot arm.
    ...
    def staged_rewards(self, action=None):
        ...
        cubeD_pos = self.sim.data.body_xpos[self.cubeD_body_id]
        cubeE_pos = self.sim.data.body_xpos[self.cubeE_body_id]
        cubeF_pos = self.sim.data.body_xpos[self.cubeF_body_id]

        cubeA_height = cubeA_pos[2]
        cubeB_height = cubeB_pos[2]
        cubeC_height = cubeC_pos[2]
        cubeD_height = cubeD_pos[2]
        cubeE_height = cubeE_pos[2]
        cubeF_height = cubeF_pos[2]
        
        table_height = self.table_offset[2]
        sorted_offsets = sorted([
            cubeA_height - table_height,
            cubeB_height - table_height,
            cubeC_height - table_height,
            cubeD_height - table_height,
            cubeE_height - table_height,
            cubeF_height - table_height,
        ])

        lower_layer_offset = (0.04 / 2)
        middle_layer_offset = 0.04 + (0.04 / 2)
        higher_layer_offset = 0.04 + (0.04 / 2) * 2
        lower_two = [offset - lower_layer_offset for offset in sorted_offsets[:2]]
        middle_two = [offset - higher_layer_offset for offset in sorted_offsets[2:4]]
        higher_two = [offset - higher_layer_offset for offset in sorted_offsets[4:]]

        scaled_r_lower = [-(offset / 0.04) if offset != 0.0 else 0 for offset in lower_two]
        scaled_r_middle = [-(offset / 0.04) if offset != 0.0 else 0 for offset in middle_two]
        scaled_r_higher = [-(offset / 0.04) if offset != 0.0 else 0 for offset in higher_two]
        reward = sum(scaled_r_lower + scaled_r_middle + scaled_r_higher)
    ...
    def _load_model(self):
        ...
        cube_size_max = 0.02
        num_colors = 2
        color_samples = random.sample(list(COLORNAME2RGBA.items()), num_colors)
        ...
        self.blocks = [self.cubeA, self.cubeB, self.cubeC, self.cubeD, self.cubeE, self.cubeF]
        ...
        if not self.deterministic_reset:
            ...
            for obj in self.blocks:
                for geom_name in self.obj_to_visual_geoms.get(obj, []):
                    ...
                    if "cubeA" in geom_name or "cubeB" in geom_name or "cubeC" in geom_name:
                        self.sim.model.geom_rgba[geom_id] = color_samples[0]
                    else:    # "cubeD" in geom_name or "cubeE" in geom_name or "cubeF" in geom_name
