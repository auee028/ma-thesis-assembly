...
class BigTower(ManipulationEnv):
    """
    This class corresponds to the customized BigTower task for a single robot arm.
    ...
    def staged_rewards(self, action=None):
        ...
        for i, offset in enumerate(sorted_offsets):
            layer_offset = 0.04 * i + (0.04 / 2)
            if offset - layer_offset == 0.0:
                scaled_r = 1
            else:
                scaled_r = -abs(offset - layer_offset) / 0.04
            reward += scaled_r
    ...
    def _load_model(self):
        ...
        self.cubeE = BoxObject(
            ...
            size_min=[cube_size_min, cube_size_min, cube_size_min],  # [0.015, 0.015, 0.015],
            size_max=[cube_size_max, cube_size_max, cube_size_max],  # [0.018, 0.018, 0.018])
            rgba=color_samples[4][1],
        )
        self.blocks = [self.cubeA, self.cubeB, self.cubeC, self.cubeD, self.cubeE]
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [cubeA_pos, cubeA_quat, cubeB_pos, cubeB_quat, cubeC_pos, cubeC_quat, 
                       cubeD_pos, cubeD_quat, cubeE_pos, cubeE_quat]
            ...
            sensors += [
                ...
                for cube in ["cubeA", "cubeB", "cubeC", "cubeD", "cubeE"]
            ]
