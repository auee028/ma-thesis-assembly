...
class Tower(ManipulationEnv):
    """
    This class corresponds to the customized Tower task for a single robot arm.
    ...
    def staged_rewards(self, action=None):
        ...
        upper_layer_offset = 0.04 * 2 + (0.04 / 2)
        
        lower_gap = sorted_offsets[0] - lower_layer_offset
        middle_gap = sorted_offsets[0] - middle_layer_offset
        upper_gap = sorted_offsets[0] - upper_layer_offset

        scaled_r = [-(offset / 0.04) if offset != 0.0 else 0 for offset in [lower_gap, middle_gap, upper_gap]]
        reward = sum(scaled_r)
