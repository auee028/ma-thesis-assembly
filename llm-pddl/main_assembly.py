...
class Assembly(Domain):
    name = "assembly" # this should match the directory name
...
class Planner:
    ...
    def create_llm_ic_pddl_prompt(self, task_nl, domain_pddl, context):
        ...
        if context_nl.startswith("### Example:"):
            # Split each context into individual examples
            nl_examples = [ex.strip() for ex in context_nl.split("### Example:")[1:]]
            pddl_examples = [ex.strip() for ex in context_pddl.split("### Example:")[1:]]
            sol_examples = [ex.strip() for ex in context_sol.split("### Example:")[1:]]
            assert len(nl_examples) == len(pddl_examples) == len(sol_examples)

            # Build the examples section
            examples = ""
            for i in range(len(nl_examples)):
                examples += f"The description of the planning problem {i+1} is: \n {nl_examples[i]} \n"
                examples += f"The problem PDDL file to this problem is: \n {pddl_examples} \n"

            prompt = f"I want you to solve planning problems. " + \
                     f"Here are some example planning problems.\n" + \
                     examples + \
                     f"Now I have a new planning problem and its description is: \n {task_nl} \n" + \
                     f"Provide me with the problem PDDL file that describes " + \
        ...
        # prompt += " Return the PDDL in plaintext format. Disable markdown rendering."
        
        return prompt

    def query(self, prompt_text, top_p=1.0):
        server_flag = 0
        server_cnt = 0
        result_text = ""
        while server_cnt < 10:
    ...
    def parse_result(self, pddl_string):
        ...
        if '```pddl' in pddl_string:
            beg = pddl_string.find("```pddl") + 7
            pddl_string = pddl_string[beg: beg + pddl_string[beg:].find("```")]
...
def llm_ic_pddl_planner(args, planner, domain):
    ...
    raw_result         = planner.query(prompt, args.top_p)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--domain', type=str, choices=DOMAINS, default="assembly")
